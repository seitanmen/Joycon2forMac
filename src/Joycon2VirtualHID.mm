...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSLog(@"HID event injection ready");
...
- (void)startEmulation {
    ...
    NSLog(@"Starting virtual HID emulation");
...
#ifndef HID_ONLY
- (void)sendHIDReportFromJoyconData:(NSDictionary *)joyconData {
    NSLog(@"Sending HID report from Joy-Con data");
    // Send Game Controller HID report (using libusb placeholder)
    ...
    // Send Gamepad HID event using CGEvent (simplified)
    // Note: Full gamepad support requires more complex implementation
    // For now, log only
    ...
    // Send Mouse HID report (extended to 7 bytes for 16-bit deltas)
